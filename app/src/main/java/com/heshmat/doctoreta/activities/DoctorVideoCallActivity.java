...
public class DoctorVideoCallActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(LOG_TAG, "patientVideoCAllChannel : "+Doctor.currentLoggedDoctor.getId()+"_"+remoteUserId);
    ...
    public void onjoinChannelClicked(View view) {
        ...
        hashMap.put("clientId", remoteUserId);
        hashMap.put("doctorId", Doctor.currentLoggedDoctor.getId());
        ...
        tokenTask.addOnSuccessListener(new OnSuccessListener<HashMap<String, Object>>() {
            ...
            public void onSuccess(HashMap<String, Object> stringObjectHashMap) {
                ...
                if (stringObjectHashMap!=null&&stringObjectHashMap.containsKey("result")&& stringObjectHashMap.get("result").equals("SUCCESSFUL")){
                    ...
                    if (!token.isEmpty()) {
                        mRtcEngine.joinChannel(token, Doctor.currentLoggedDoctor.getId()+"_"+remoteUserId, "Extra Optional Data", 0); // if you do not specify the uid, Agora will assign one.
                        Log.i(LOG_TAG, "doctor: "+String.format("%s_%s",Doctor.currentLoggedDoctor.getId(),remoteUserId));
                        setupLocalVideoFeed();
            ...
            public void onFailure(@NonNull Exception e) {
                loadingDialog.dismissDialog();
                new AlertDialog.Builder(DoctorVideoCallActivity.this).setMessage(e.getMessage()).show();
            }
        });
